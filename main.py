...
def no_projects(message):
    """Сообщает пользователю, что у него нет проектов."""
...
/projects - посмотри все свои проекты
/delete - удали проект
/update_projects - обнови информацию о проекте
Также ты можешь ввести имя проекта и узнать информацию о нем!
""")

@bot.message_handler(commands=['new_project'])
def addtask_command(message):
    """Запускает процесс добавления нового проекта."""
    bot.send_message(message.chat.id, "Введите название проекта:")
    bot.register_next_step_handler(message, name_project)

def name_project(message):
    """Сохраняет название проекта и запрашивает ссылку."""
    name = message.text
...
def callback_project(message, data, statuses):
    """Сохраняет статус проекта и добавляет проект в базу данных."""
    status = message.text
...
def skill_handler(message):
    """Запрашивает навыки для проекта."""
    user_id = message.from_user.id
...
def skill_project(message, projects):
    """Выбирает проект для добавления навыка."""
...
def get_projects(message):
    """Отправляет список проектов пользователя."""
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
...
def delete_handler(message):
    """Запускает процесс удаления проекта."""
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
        text = "\n".join([f"Project name: {x[2]} \nLink: {x[4]}\n" for x in projects])
...
def update_project(message):
    """Запускает процесс обновления проекта."""
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]
...
def update_project_step_2(message, projects):
    """Выбирает проект для обновления."""
    project_name = message.text
    if message.text == cancel_button:
        cansel(message)
        return
...
def update_project_step_3(message, project_name):
    """Запрашивает, что нужно изменить в проекте."""
